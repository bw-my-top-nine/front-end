...
function Home(props) {
	...
	useEffect(() => {
		axios
			.get(`https://top-nine.herokuapp.com/api/categories/${userId}/categories`)
			.then(resp => {
				console.log(resp);
		...
		<section>
			<Route
				...
				render={props => (
					<MyLists
						{...props}
						categories={userLists}
						setEdit={setEdit}
						userId={userId}
					/>
				)}
			/>{" "}
			{/* to do: pass data/handlers from axios/useState */}
			<Route
				path="/home/createcategoryform"
				render={props => (
					<CreateCategory {...props} edit={edit} handleSubmit={handleSubmit} />
				)}
			/>{" "}
			{/* to do: pass data/handlers from axios/useState */}
		</section>
